...
export class CustomButton extends LitElement {
	...
	renderError() {
		...
		if (this.validation instanceof Array) {
			validations = this.validation.map((validation: string) => i18next.t(`validation.${validation}`, { field }));
		} else if (this.validation) {
			validations.push(i18next.t(`validation.${this.validation}`, { field }));
		}
		return html`<div class="error">${validations.map((validation) => html`${validation}`)}</div>`;
	}
